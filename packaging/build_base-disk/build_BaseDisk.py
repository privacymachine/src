...
def build_BaseDisk(build_config,base_disk,logger):
    ...
    #define VM handler
    buildVM = VM('root','root',build_config['VM']['VM_name'],build_config['paths']['grml_ssh_priv_key'],'22','grml.lan',logger,build_config['VM']['boot_delay'])
    buildVM.stop()

    # clean build-VM
    returns.append(cmd_exec('dd',['if=/dev/zero','bs=1G','count='+str(build_config['VM']['disk_size']),'of='+build_config['VM']['disk_path']],logger))
...
#        returns.append(buildVM.sshcmd('grml-chroot /target apt install -y apt-listchanges 2>&1 | tee -a /apt.log'))
        ...
        buildVM.copyFiles2Host('/apt.log',base_disk['log_dir']+'/apt.log')
        buildVM.copyFiles2Host('/grml-debootstrap.log',base_disk['log_dir']+'/grml-debootstrap.log')
        buildVM.stop()
        ...
        # define VM to install VirtualBox guest additions
        returns.append(cmd_exec('virsh',['undefine',build_config['VM']['VM_name']],logger))
        returns.append(cmd_exec('virsh',['define',build_config['paths']['guestAdditions_xml']],logger))
        ...
        if buildVM.isonline():
            ...
            returns.append(buildVM.sshcmd('mount /dev/cdrom /mnt'))
            ...
            returns.append(buildVM.sshcmd('su liveuser -c /pm/download_tor_browser.sh'))
            ...
            buildVM.copyFiles2Host('/timezones.txt',base_disk['image_dir']+'/timezones.txt')
            buildVM.copyFiles2Host('/locales.txt',base_disk['image_dir']+'/locales.txt')

             # copy the log file
            buildVM.copyFiles2Host('/post_build.log',base_disk['log_dir']+'/post_build.log')
            
            # copy base-disks ssh-host-pub-key
            buildVM.copyFiles2Host('/etc/ssh/ssh_host_rsa_key.pub',base_disk['image_dir']+'/ssh_host_rsa_key.pub')
            ...
            #TODO: persist ssh-host keys
                        
            # clean ssh and stop the VM   
            #returns.append(buildVM.sshcmd('rm -r /root/.ssh && shutdown -hP 0'))
            returns.append(buildVM.sshcmd('shutdown -hP 0'))
            time.sleep(10)
            buildVM.stop()
            ...
            if build_config['keep_tmp_files']:
                with open(base_disk['image_dir']+'/fonts.txt', 'w') as f:
                    for l in installed_fonts:
                        f.write(l+'\n')
    
    # check if anything went wrong
    if all(map(lambda x:x==0,returns)):
        logger.info('>>>>>>>>>>>>>   SUCCESS: installing debian   >>>>>>>>>>>>>>')
        logger.info('copy finished base-disk')
        image_name = base_disk['image_identifier']+'_flat.vmdk'
        base_disk['flat_vmdk_path'] = base_disk['image_dir'] +'/'+ image_name
        cmd_exec('cp',[build_config['VM']['disk_path'], base_disk['flat_vmdk_path']] ,logger)
                        
        capabilities['version'] = base_disk['version']
                                   
        logger.info('collect data for capabilities.json')   
        with open(base_disk['image_dir']+'/ssh_host_rsa_key.pub','r') as f:
            capabilities['ssh_keys']={'BaseDisk_pub':f.read()}
        if not build_config['keep_tmp_files']:
            cmd_exec('rm',[base_disk['image_dir']+'/ssh_host_rsa_key.pub'],logger)
        with open(buildVM.sshcmdkey,'r') as f:
            capabilities['ssh_keys']['PM_priv']=f.read()
        timezones=[]
        with open(base_disk['image_dir']+'/timezones.txt','r') as f:
            for line in f:
                timezones.append(line.strip('\n'))
        capabilities['timezones']=timezones
        if not build_config['keep_tmp_files']:
            cmd_exec('rm',[base_disk['image_dir']+'/timezones.txt'],logger)
        locales=[]
        with open(base_disk['image_dir']+'/locales.txt','r') as f:
            for line in f:
                locales.append(line.strip('\n'))
        capabilities['locales']=locales
        if not build_config['keep_tmp_files']:
            cmd_exec('rm',[base_disk['image_dir']+'/locales.txt'],logger)

        logger.info('write capabilities.json')  
        base_disk['capabilities_path'] = base_disk['image_dir'] + '/' + base_disk['image_identifier']+'_capabilities.json'
        with open(base_disk['capabilities_path'],'w') as f:
            json.dump(capabilities,f)
            
        logger.info('zerofree'+ base_disk['flat_vmdk_path'])
        cmd_exec('losetup',['-D'],logger)
        cmd_exec('losetup',['-o',str(2048*512),'/dev/loop0',base_disk['flat_vmdk_path']],logger)
        cmd_exec('zerofree',['/dev/loop0'],logger)
        cmd_exec('losetup',['-D'],logger)
        
        logger.info('>>>>>>>>>>>>>>>>>>   SUCCESS: base-disk builded    >>>>>>>>>>>>>>>>>>')
        
        return base_disk
        
    else:
        logger.fatal('A error occured building the base-disk')
        logger.fatal('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
