...
def copyFile2DeliverServer(source, target, ssh_data,logger):
    ...
    try:
        ...
        logger.fatal('could not copy '+source+' to '+ssh_identify_str+ \
                        '\n cmd: '+str(err.cmd)+'\n output: '+ \
                        str(err.output) + '\n returncode: ' +\
                        str(err.returncode))
        return err.returncode,err
    ...
    cmd = ['sshpass','-p', ssh_data['ssh_pw'],
           'ssh',ssh_data['ssh_options'], ssh_data['ssh_host'], 
           'chmod', '644', ssh_data['ssh_home']+'/'+target]
    cmd = list(flatten(cmd))
    try:
        sp.check_output(cmd, stderr=sp.STDOUT)    
    except sp.CalledProcessError as err:
        logger.fatal('cold not set permissions to 644 for '+ \
                        ssh_data['ssh_host']+':'+ssh_data['ssh_home']+'/'+target +\
                        '\n cmd: '+str(err.cmd)+'\n output: '+ \
                        str(err.output) + '\n returncode: ' + \
                        str(err.returncode))
...
def hash_and_sign(folder,file_prefix,keyID,logger,additional_files=[],sign=True):
...
if __name__ == '__main__':
    ...
    try:        
        ...
        print('Use generateInitialConfig.py to create it.')
        print()
        exit(1)        
...
# Increment ComponentMajor (BaseDisk's main version number)
    config['actual_version']['ComponentMajor'] += 1  

# Create new BaseDisk context
    base_disk = {'version':config['actual_version']}

    # create output folder; name based on ComponentMajor version number
    base_disk['image_identifier']='BaseDisk_'+str(base_disk['version']['ComponentMajor'])
    ...
    fh = logging.FileHandler(base_disk['log_dir']+'/build_BaseDisk.log')
    ...
    if True:    
        ...
        logger.info('create BaseDisk description file '+ base_disk['vmdk_path'])
        ...
        if config["gpg_sign"]:
            ...
            hash_and_sign(base_disk['image_dir'],base_disk['image_identifier'],config['signing_key'],logger,sign=config['gpg_sign'])
...
#        base_disk['zip_path']= base_disk['image_dir']+'.7z'
        ...
        if config['upload_zips']:
            ...
            copyFile2DeliverServer(base_disk['zip_path'],base_disk['image_identifier']+'.7z',config['delivery_server'],logger)
    
    base_disk['deltas'] = []       
    
    if len(config['BaseDisks']) > 0:
        # check for old base-diks 
        if len(config['BaseDisks']) > config['max_BaseDisks']:
            logger.info('There are '+str(len(config['BaseDisks']))+ \
                ' BaseDisks.\nDelete oldest '+str(len(config['BaseDisks']) \
                - config['max_BaseDisks'])+' BaseDisks.')
            
            while len(config['BaseDisks']) > config['max_BaseDisks']:
                to_remove = config['BaseDisks'].pop(0)
        ...
        for old_base_disk in  config['BaseDisks']:
        # build new delta
            delta_info={}
            delta_info['identifier'] = 'BaseDisk_delta_' + str(old_base_disk['version']['ComponentMajor']) + '-' + str(base_disk['version']['ComponentMajor'])
            ...
            if config["gpg_sign"]:
                ...
                hash_and_sign(delta_info['dir_path'],delta_info['identifier'],config['signing_key'],logger,[base_disk['flat_vmdk_path']],sign=config['gpg_sign'])
...
#            delta_info['zip_path']= delta_info['dir_path']+'.7z'
            ...
            if config['upload_zips']:
                ...
                copyFile2DeliverServer(delta_info['zip_path'],delta_info['identifier']+'.7z',config['delivery_server'],logger)

            base_disk['deltas'].append(delta_info)
            
    else:
        logger.info('No old BaseDisks, nothing to do.') 
        
    # write config and exit        
    config['BaseDisks'].append(base_disk)
