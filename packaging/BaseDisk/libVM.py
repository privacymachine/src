...
class VM:
    ...
    def isonline(self):   
        ...
        for n in range(maxTries):
            ...
            tmp = self.sshcmd('echo test',True,showErrors=False)
            if tmp[0]==0:
    ...
    def sshcmd(self,cmd,returnOutput=False,showErrors=True):
    ...
    def start(self):    
        ...
        try:
            str1 = sp.check_output(['/usr/bin/virsh','start',
                                    self.vmname],
                                    stderr=sp.STDOUT).decode("utf-8")
            self.logger.info('Successfully started the VM '+self.vmname+', now wait '+str(self.bootDelay)+' seconds to boot it.')                                    
            time.sleep(self.bootDelay)
        except sp.CalledProcessError as err:
    ...
    def stop(self):
        if self.online or (not self.online and self.isonline()):
            ...
            self.sshcmd('shutdown -hP now',showErrors=False)
            time.sleep(self.bootDelay)
            self.isonline()
        return 0
