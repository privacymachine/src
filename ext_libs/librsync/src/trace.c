...
static void rs_log_va(int level, char const *fn, char const *fmt, va_list va);
  ...
  #if (WIN32)
    #pragma message("warning size_t is larger than a long integer, values in trace messages may be wrong")
  #else
    #warning "warning size_t is larger than a long integer, values in trace messages may be wrong"
  #endif
#endif
