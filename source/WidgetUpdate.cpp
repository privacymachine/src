...
bool WidgetUpdate::init(PMManager* manager)
{
  ...
  manager->createCommandsForUpdateAllUsecases(allCommands);

  if (!widgetCommandExec_->init(allCommands)) // widgetCommandExec_ also deletes the memory of the commands
    return false;
